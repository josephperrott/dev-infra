load("//ng-dev/benchmark/ng_rollup_bundle:ng_rollup_bundle.bzl", "ng_rollup_bundle")
...
def copy_default_file(origin, destination):
    ...
    native.genrule(
        ...
        srcs = ["//ng-dev/benchmark/component_benchmark/defaults:" + origin],
        outs = [destination],
        cmd = "cat $(SRCS) >> $@",
    )
...
def component_benchmark(
    ...
    ** USAGE NOTES **
    ...
    ng_module(
        ...
        deps = ng_deps,
        tsconfig = "//ng-dev/benchmark/component_benchmark:tsconfig-e2e.json",
    )
    ...
    concatjs_devserver(
        ...
        additional_root_paths = ["//ng-dev/benchmark/component_benchmark/defaults"],
