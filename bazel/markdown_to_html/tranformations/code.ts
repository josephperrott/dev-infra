...
type DocEntry = any;
type EntryType = any;
import {handleEmoji} from './emoji.js';
...
function getApiEntries(): Map<string, Array<ParsedEntry>> {
  ...
  entryCollections.forEach((module) => {
    ...
    module.entries.forEach((entry) => {
      const members = (entry as ClassEntry).members?.map((m: any) => m.name) ?? [];
      const parsedEntry = {
        module: moduleName,
        members,
        type: entry.entryType,
        isDeprecated: entry.jsdocTags.some((tag: any) => tag.name === 'deprecated'),
      };
