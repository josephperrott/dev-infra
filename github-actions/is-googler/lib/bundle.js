...
async function labelAsGoogler(client, label) {
...
async function run() {
    try {
        const label = core_5('label', { required: true });
        // Github App private key
        const privateKey = core_5('bot-key', { required: true });
        const githubApp = new distNode_1({ id: 6359, privateKey });
        ...
        const isMemberOfAngularOrg = await client.orgs
            ...
            .then(() => true, () => false);
        console.log(`Item created by ${github_1.actor}`);
        ...
        if (!isMemberOfAngularOrg && isMemberOfGoogleOrg) {
            console.log(`Adding label '${label}' to item #${github_1.issue.number}`);
...
if (github_1.repo.owner === 'angular') {
    ...
    core_10(`Skipping, the is-googler github action is only meant to be run on repos in the Angular org.`);
}
