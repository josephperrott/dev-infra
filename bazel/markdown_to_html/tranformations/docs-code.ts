...
import {EXAMPLES_FOLDER_PATH, TUTORIALS_FOLDER_PATH} from '../constants.js';
import {DocumentType, getCurrentParsedDocumentType} from '../state.js';
...
function getCodeFromPath(path: string): string {
  console.log('something', `${runfiles.resolveWorkspaceRelative('bazel/markdown_to_html/test')}`);
  throw Error();
  const rootFolder =
    getCurrentParsedDocumentType() === DocumentType.DOCS
      ? EXAMPLES_FOLDER_PATH
      : TUTORIALS_FOLDER_PATH;
  return readFileSync(join(runfiles.runfilesDir!, path), {encoding: 'utf-8'});
}
