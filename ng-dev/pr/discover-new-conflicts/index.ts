...
export async function discoverNewConflictsForPr(newPrNumber: number, updatedAfter: number) {
  ...
  git.run(['fetch', requestedPr.headRef.repository.url, requestedPr.headRef.name]);
  git.run(['checkout', '-B', tempWorkingBranch, 'FETCH_HEAD']);

  // Rebase the PR against the PRs target branch.
  git.run(['fetch', requestedPr.baseRef.repository.url, requestedPr.baseRef.name]);
  try {
    git.run(['rebase', 'FETCH_HEAD']);
  } catch (err) {
  ...
  for (const pr of pendingPrs) {
    ...
    git.run(['fetch', pr.headRef.repository.url, pr.headRef.name]);
    git.run(['checkout', '--detach', 'FETCH_HEAD']);
    // Check if the PR cleanly rebases into the repo after the target PR.
    try {
      git.run(['rebase', tempWorkingBranch]);
    } catch (err) {
      if (err instanceof GitCommandError ) {
        conflicts.push(pr);
      }
